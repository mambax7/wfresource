...
class XoopsFormSelectImage extends XoopsFormSelect {
	 ...
	 */
	function XoopsFormSelectImage( $caption, $name, $value = null, $id = 'Xoops_image', $imgcat_id, $size = 5 ) {
	...
	function render() {
		...
		if ( $this->getValue() ) {
			...
			if ( count( $image_array ) == 1 ) {
				$image_size = @getimagesize ( XOOPS_ROOT_PATH . '/' . $this->getCategory() . '/' . $this->getValue() );
