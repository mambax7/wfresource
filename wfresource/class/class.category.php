...
class wfp_CategoryHandler extends wfp_ObjectHandler {
	 ...
	 * @return
	 */
	function wfp_CategoryHandler( &$db ) {
	 ...
	 * @return
	 */
	function getRead_permissions( &$obj, $type = 'wfp_category_read' ) {
	 ...
	 * @return
	 */
	function getSave_permissions( &$obj, $type = 'wfp_category_read', $value = null ) {
		if ( !is_array( $value ) ) {
			return false;
		}
		$_group = &wfp_getClass( 'permissions' );
	...
	function headingHtml( $value, $total_count ) {
		...
		$ret .= '<form>
		 <div style="text-align: left; margin-bottom: 12px;">
		  <input type="button" name="button" onclick=\'location="admin.category.php?op=edit"\' value="' . _MA_WFP_CREATENEW . '">
		  <input type="button" name="button" onclick=\'location="admin.category.php?op=permissions"\' value="' . _MA_WFP_PERMISSIONS . '">
		 </div></form>';
		$ret .= '<div>
			<span style="float: right">' . _MA_WFP_DISPLAYAMOUNT_BOX . wfp_getSelection( wfp_ListArray(), $nav['limit'], 'limit', 1, 0, false, false, sprintf( $onchange, 'limit' ) , 0, false ) . '</span>
			</div>';
