...
class wfp_Tlist {
	 ...
	 * @return
	 */
	function setPrefix( $value = null ) {
		$this->_pre_fix = ( $value != null ) ? strval( $value ) : '_MA_';
	}
	...
	function noselection() {
		$ret = '<tr><td colspan="' . $this->_headers_count . '" class="emptylist">' . _MA_WFP_NORECORDS . '</td></tr>';
		if ( $this->_output ) {
			echo $ret;
		} else {
			return $ret;
	 ...
	 * @return
	 */
	function setSubmit( $array = array() ) {
		if ( empty( $array ) ) {
			$array = array( 'updateall' => _MA_WFC_UPDATESELECTED,
				'deleteall' => _MA_WFC_DELETESELECTED,
				'duplicateall' => _MA_WFC_DUPLICATESELECTED );
		}
