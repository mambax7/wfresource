...
function wfp_ShowPagenav( $tot_num = 0, $num_dis = 10, $start = 0, $from = 'start', $nav_type = 1, $nav_path = '', $returns = false ) {
	...
	$records = ( $tot_num > 0 ) ? sprintf( _MA_WFP_RECORDSFOUND, $from_result, $to_result, $tot_num ): _MA_WFP_NORECORDS;

	$navigation = '';
	$page = ( $tot_num > $num_dis ) ? _MA_WFP_PAGE : '';
	if ( intval( $tot_num ) > ( int )$num_dis ) {
...
function xoosla_cp_footer() {
	...
	echo '<div style="padding-top: 16px; padding-bottom: 10px; text-align: center;">
		<a href="' . $xoopsModule->getInfo( 'website' ) . '" target="_blank">' . wfp_showImage( 'wfprojects', $_title = '', '', 'gif' ) . '
		</a>
	</div>';
	xoops_cp_footer();
}
...
function wfp_doUpload( $value = 'file', $handler, $prefix = null, $uploadfolder = null ) {
	...
	foreach( $_FILES as $FILE ) {
		$uploader = new wfp_Uploader( $uploadfolder,
			$array,
			$ConfigUser['maxsize'],
			$ConfigUser['width'],
			$ConfigUser['height']
		...
		if ( !empty( $FILE['name'] ) ) {
			if ( $uploader->fetchMedia( $FILE ) ) {
				if ( $uploader->upload() ) {
					$_REQUEST['media'][] = array( $value => $uploader->getSavedFileName(),
...
function wfp_uploader( $allowed_mimetypes, $uploadfile, $redirecturl = "index.php", $num = 0, $uploaddir = "uploads", $redirect = 0 ) {
	...
	$uploader = new XoopsMediaUploader( XOOPS_ROOT_PATH . "/${uploaddir}",
		$allowed_mimetypes,
		$xoopsModuleConfig['maxfilesize'],
		$xoopsModuleConfig['maximgwidth'],
		$xoopsModuleConfig['maximgheight']
		);
	/**
	 */
	...
	redirect_header( $redirecturl, 1, _MA_WFP_FILEUPLOAD );
}
...
function wfp_ListPages() {
	return array( '0' => _MA_WFC_SELALL, '1' => _MA_WFC_SELPUBLIHSED, '2' => _MA_WFC_SELUNPUBLISHED, '3' => _MA_WFC_SELEXPIRED, '4' => _MA_WFC_SELOFFLINE );
}
